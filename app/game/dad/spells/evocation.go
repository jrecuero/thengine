...
func NewGuidingBolt() *rules.Spell {
	guidingBolt := rules.NewSpell(GuidingBoltName, "guiding bolt", constants.Evocation, 1, rules.NewDamage(rules.DiceThrow4d6, constants.Radiant))
...
On a hit, the target takes 4d6 radiant damage, and the next attack roll made
 ...
 to the mystical dim light glittering on the target until then.`)
	guidingBolt.SetComponents([]rules.MagicComponent{constants.Verbal, constants.Somatic})
	guidingBolt.SetCastingTime(1)                                                      // 1 action
	guidingBolt.SetRange(120)                                                          // TODO: 120 feet
	guidingBolt.SetDuration(1)                                                         // 1 round
	guidingBolt.SetHigherLevel(rules.NewDamage(rules.DiceThrow1d6, constants.Radiant)) // 1d6 per higher level
	return guidingBolt
}
